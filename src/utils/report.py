...
def generate_html_report(df, output_path="report.html"):
    ...
    # Extract control docking info if available
    control_info = {}
    if 'Control_Name' in df.columns and df['Control_Name'].notna().any():
        control_info['name'] = df['Control_Name'].iloc[0]
        control_info['smiles'] = df['Control_SMILES'].iloc[0] if 'Control_SMILES' in df.columns else 'N/A'
        if 'Control_vina_Score' in df.columns:
            control_info['vina'] = df['Control_vina_Score'].iloc[0]
        if 'Control_autodock_Score' in df.columns:
            control_info['autodock'] = df['Control_autodock_Score'].iloc[0]
    ...
    <body>
        ...
        <div class="container">
            ...
            <!-- Control Reference (if applicable) -->
            {'<div class="card" style="margin-bottom: 20px; border-left: 4px solid #3498db;">' if control_info else ''}
                {'<h2>📍 Control Reference - Most Similar Compound</h2>' if control_info else ''}
                {'<p style="color: #7f8c8d; margin-bottom: 10px;">Docking reference from control set (most similar to candidates)</p>' if control_info else ''}
                {'<table style="width: 100%; border-collapse: collapse;">' if control_info else ''}
                    {'<tr style="border-bottom: 1px solid #ecf0f1;">' if control_info else ''}
                        {'<td style="padding: 10px; font-weight: bold; width: 150px;">Control Name:</td>' if control_info else ''}
                        {f'<td style="padding: 10px;">{control_info.get("name", "N/A")}</td>' if control_info else ''}
                    {'</tr>' if control_info else ''}
                    {'<tr style="border-bottom: 1px solid #ecf0f1;">' if control_info else ''}
                        {'<td style="padding: 10px; font-weight: bold;">SMILES:</td>' if control_info else ''}
                        {f'<td style="padding: 10px; font-family: monospace; font-size: 11px;">{control_info.get("smiles", "N/A")[:80]}</td>' if control_info else ''}
                    {'</tr>' if control_info else ''}
                    {f'<tr style="border-bottom: 1px solid #ecf0f1;">' if control_info and 'vina' in control_info else ''}
                        {f'<td style="padding: 10px; font-weight: bold;">Vina Score:</td>' if control_info and 'vina' in control_info else ''}
                        {f'<td style="padding: 10px; color: #e74c3c; font-weight: bold;">{control_info["vina"]:.2f} kcal/mol</td>' if control_info and 'vina' in control_info else ''}
                    {f'</tr>' if control_info and 'vina' in control_info else ''}
                    {f'<tr>' if control_info and 'autodock' in control_info else ''}
                        {f'<td style="padding: 10px; font-weight: bold;">AutoDock-GPU Score:</td>' if control_info and 'autodock' in control_info else ''}
                        {f'<td style="padding: 10px; color: #e74c3c; font-weight: bold;">{control_info["autodock"]:.2f} kcal/mol</td>' if control_info and 'autodock' in control_info else ''}
                    {f'</tr>' if control_info and 'autodock' in control_info else ''}
                {'</table>' if control_info else ''}
            {'</div>' if control_info else ''}
